...
- An async `Registry` variant — **shipped**, as `pacta-contract`'s `async` feature. A durable backend
  ...
  **ready-future-only** contention check. A sync-only consumer that does not enable `async` compiles
  ...
  - **Resolved: backend-type coloring is consumer-owned.** `Registry` and `AsyncRegistry` no longer
    imply `Send + Sync`, so an `Rc`-based single-threaded backend can implement either binding and
    run the sequential conformance suite. The OS-thread contention entries state
    `Send + Sync + 'static` themselves, at the boundary that actually shares `Arc<R>`; async futures
    remain `Send`-agnostic, while the pure `Transition` closure remains `Send + Sync` independently.
    This is breaking for downstream generic code that used `R: Registry` or `R: AsyncRegistry` to
    imply thread safety: such callers add `Send + Sync` where they actually cross threads. Version
    and release bookkeeping remain a separate release-finalization change.
