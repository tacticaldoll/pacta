...
const KERNEL_NO_SERDE_REASON: &str = "the sans-I/O kernel is transient driving protocol, not durable state: no type defined in it acquires Serialize/Deserialize, by derive or by a hand-written impl whose self type resolves to it, so the kernel offers no serde path for persisting an in-flight directive or notice. Durable records (Pact, Claim, Retainer, Timestamp) carry serde; the kernel's types must not. Coverage is partial by nature: a macro-generated impl, and a hand-written impl whose self type cannot be resolved (for example one reached through a glob import), are invisible to a source scan.";
const CORE_NO_IO_REASON: &str = "the sans-I/O core contract performs no I/O: no code in pacta-contract (the kernel included) makes an inline call into a std::io/fs/net/process path; I/O lives in runtimes and backends outside the core. Coverage is partial by nature (I/O entry points cannot be enumerated, and a call through a method on a value, such as `write_all` on a writer, or macro-expanded I/O such as println! is invisible to a source scan), so this tooth complements review rather than replacing it.";
const MEMORY_REASON: &str = "pacta-memory is a registry backend outside the core. Its normal dependencies are limited to pacta-contract and uuid, never drivers, executors, or other backends.";
const CONFORMANCE_REASON: &str = "pacta-conformance is a backend-agnostic test suite. Its normal dependencies are limited to the contract it verifies — pacta-contract (whose `async` feature carries the async binding it also exercises) — plus uuid, never a specific backend.";
const PROSE_REASON: &str =
    "active prose must not reintroduce stale architecture-defining vocabulary";
const AMBIENT_TIME_REASON: &str = "the core contract makes no inline `std::time` `now` call (such as `SystemTime::now()` or `Instant::now()`), because time is injected at the Registry seam. Coverage is partial by nature (a clock read through a method on a value, such as `Instant::elapsed`, or a `now` path taken as a value rather than called, is invisible to a source scan), so this tooth complements review rather than replacing it.";
const AMBIENT_TIME_UUID_REASON: &str = "the core contract makes no inline `uuid` `now_v7` or `now_v1` call, even fully qualified, so it mints no identifier from the ambient clock through those constructors. Coverage is partial by nature (a time-reading uuid path outside that pair, a path taken as a value, and an `extern crate … as` rename are invisible to a source scan), so this tooth complements review rather than replacing it.";
const FACADE_REASON: &str = "pacta is the curated published entrypoint. Its normal dependencies are limited to pacta-contract, pacta-executor, and pacta-driver, never a backend or external framework.";
