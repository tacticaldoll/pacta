...
The core owns the mechanism; you own the policy. Orchestration such as retry,
timeout, or backoff is not shipped — it composes onto the `Middleware` seam. You run
your `Registry` and `Executor` with the reference `Driver`, or compose your own loop
over the kernel.
